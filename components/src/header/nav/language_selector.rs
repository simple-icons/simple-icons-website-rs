...
use crate::storage::{set_on_localstorage, LocalStorage};
use crate::Url;
...
fn initial_language() -> &'static Language {
    match Url::params::get(&Url::params::Names::Language) {
        Some(value) => {
            let lang = <&'static Language>::from_str(value.as_str())
                .unwrap_or(<&'static Language>::default());
...
fn initial_language_from_localstorage() -> Option<&'static Language> {
    match window()
        ...
        .unwrap()
        .unwrap()
        .get_item(LocalStorage::Keys::Language.as_str())
    {
        Ok(Some(lang)) => <&'static Language>::from_str(lang.as_str()).ok(),
        _ => None,
    }
}
