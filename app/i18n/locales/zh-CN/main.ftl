...
clear-search = 清除搜索
maintained-by = 由 { $maintainers } 维护的 { $license } 项目。
cco = CC0
github = GitHub
open-collective = Open Collective
simple-icons-contributors = Simple Icons 的贡献者
use-platform = 在 { $platform } 提交申请、更正和贡献。
...
copy-alias-aka = 复制 { $icon } AKA（{ $alias }）
copy-alias-dup = 复制 { $icon } 重复的品牌（{ $alias }）
copy-alias-loc = 复制 { $icon } 本地化名称 ({ $lang })（{ $alias }）
copy-alias-old = 复制 { $icon } 旧名称（{ $alias }）
